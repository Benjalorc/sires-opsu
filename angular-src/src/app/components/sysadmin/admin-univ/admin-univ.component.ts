...
///////////
  ...
  eliminarUniversidad(deleted){

    alert("BORRADA LA "+deleted)
    let univs = this.universidades.filter((element) =>{return element.codigo != deleted});
  ...
  dibujarGrafica(info){
    ...
    this.universidades.forEach((element) =>{
      total += element[info.toLowerCase()];
    });

    this.universidades.forEach((element) =>{
      let dato = {
        etiqueta: element.siglas,
        valor: element[info.toLowerCase()]
      }
  ...
  seleccionarCarrera(codigo){
    ...
    console.log(carr);
    for(let i = 0, j = this.carreras.length; i<j; i++){

      if(this.carreras[i].codigo == codigo){
        this.carreraActual = this.carreras[i];
        this.codigo_int = ""+this.universidadActual.codigo+""+this.carreraActual.codigo;
        this.nombre_carr = this.carreraActual.nombre;
